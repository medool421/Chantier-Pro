...
import { useProject, useUpdateProjectStatus } from '../../../src/hooks/useProjects';
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity style={styles.statusContainer} onPress={() => setStatusModalVisible(true)}>
            <View style={[styles.statusBadge, { backgroundColor: getStatusColor(project.status) }]}>
              <Text style={styles.statusText}>{PROJECT_STATUS_LABELS[project.status] || project.status}</Text>
            </View>
            <Ionicons name="chevron-down" size={16} color={colors.textMuted} />
          </TouchableOpacity>

          <View style={styles.dateRow}>
            <Text style={styles.dateText}>Début: {new Date(project.startDate).toLocaleDateString('fr-FR')}</Text>
            {project.endDate && (
              <Text style={styles.dateText}>Fin: {new Date(project.endDate).toLocaleDateString('fr-FR')}</Text>
            )}
          </View>
          ...
          {project.budget && (
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoText}>Budget: {project.budget.toLocaleString('fr-MA')} DH</Text>
            </View>
          )}
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity
            ...
            onPress={() => router.push({ pathname: `/(manager)/projects/${id}/tasks/create`, params: { projectId: id } })}
          >
          ...
          <TouchableOpacity
            style={styles.actionCard}
            onPress={() => router.push({ pathname: `/(manager)/projects/${id}/tasks`, params: { projectId: id } })}
          >
          ...
          <View style={styles.modalContent}>
            ...
            {Object.entries(PROJECT_STATUS).map(([key, value]) => (
              ...
              >
                ...
                {project.status === value && <Ionicons name="checkmark" size={20} color={colors.primary} />}
              </TouchableOpacity>
            ))}
...
const getStatusColor = (status) => ({
  PLANNED: '#E3F2FD', IN_PROGRESS: '#C8E6C9', ON_HOLD: '#FFF3E0', COMPLETED: '#F3E5F5',
}[status] || '#F5F5F5');
...
const styles = StyleSheet.create({
  ...
  projectTitle: { fontSize: 24, fontWeight: 'bold', color: colors.textDark, marginBottom: 12 },
